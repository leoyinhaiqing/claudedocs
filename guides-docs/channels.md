...
View the full Telegram plugin source at: https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins/telegram

**Steps to set up Telegram:**

1. **Create a Telegram bot**
   - Open BotFather in Telegram and send `/newbot`
   - Give it a display name and a unique username ending in `bot`
...
3. **Configure your token**
   - Run the configure command with the token from BotFather:
     ...
     ```
   - This saves it to `.claude/channels/telegram/.env` in your project
   - You can also set `TELEGRAM_BOT_TOKEN` in your shell environment before launching Claude Code
...
View the full Discord plugin source at: https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins/discord

**Steps to set up Discord:**
...
2. **Enable Message Content Intent**
   - In your bot's settings, scroll to **Privileged Gateway Intents**
   - Enable **Message Content Intent**

3. **Invite the bot to your server**
   - Go to **OAuth2 > URL Generator**
   - Select the `bot` scope and enable these permissions:
     * View Channels
     * Send Messages
...
4. **Install the plugin**
   - In Claude Code, run: `/plugin install discord@claude-plugins-official`
...
5. **Configure your token**
   - Run the configure command with the bot token you copied:
     ...
     ```
   - This saves it to `.claude/channels/discord/.env` in your project
   - You can also set `DISCORD_BOT_TOKEN` in your shell environment before launching Claude Code
...
3. **Push a message in**
   - Open the fakechat UI at http://localhost:8787 and type a message:
     ...
     ```
   - The message arrives in your Claude Code session as a `<channel source="fakechat">` event
   - Claude reads it, does the work, and calls fakechat's `reply` tool
   - The answer shows up in the chat UI
...
Report issues or feedback on the Claude Code GitHub repository at: https://github.com/anthropics/claude-code/issues
